...
# Data In ----------------------------------------------------------------
data.in <- read_excel("~/Documents/GitHub/TAT-Investigation/data/PEST06.xlsx", 
...
test <- substr(data.in$ANALYSIS[1], 1,6)
...
prep <- read_excel("~/Documents/GitHub/TAT-Investigation/data/ALL_PREP.xlsx", col_types = c("numeric", 
...
set_plot_1 <- ggplot(set, aes(x=Waiting)) +
        ...
        labs(title = paste("Start Times for", test, sep=" "), subtitle = "Days from Receipt at the Laboratory to Commencing Testing", x="Days", y="Samples") +
        theme_bw() +
...
set_plot_3 <- ggplot(set, aes(x=Checked)) +
        geom_histogram(binwidth = 0.2, fill = "cornflowerblue", colour = "grey85")+
        labs(title = paste("Release Times for", test, sep = " "), subtitle = "Days from Completing Testing to Releasing", x="Days", y="Samples") +
        theme_bw() +
        theme(panel.grid.major = element_line(size = 0.5, color = "grey"), 
              axis.line = element_line(size = 0.7, color = "black"), 
              text = element_text(size = 10))
...
set_plot_2 <- ggplot(set, aes(x=TAT)) +
  geom_histogram(binwidth = 0.2, fill = "cornflowerblue", colour = "grey85")+
  labs(title = paste("Overall TAT for", test, sep = " "), subtitle = "Days from Logging the Sample to Reviewing the Sample", x="Days", y="Samples") +
  theme_bw() +
  theme(panel.grid.major = element_line(size = 0.5, color = "grey"), 
        axis.line = element_line(size = 0.7, color = "black"), 
        text = element_text(size = 10))
...
* Prep  = time from receipt to release from sample prep.
* Waiting = time from being released from sample prep to being started.
* Test_Time = time from being started to being completed.
* Checked = time from being completed to being reviewed.
* Review = time to be reviewed once released.
...
df <- data.frame(set[,c(2:9)])
cols <- c('Transit', 'Prep', 'Waiting', 'Test_Time', 'Checked', 'Review', 'TAT')
stargazer(
    df[, cols], type = "text", 
    summary.stat = c("n", "mean", "min", "median", "max")
)
